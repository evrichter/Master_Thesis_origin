...
p4 <- p4 + geom_point(data = Effect_sizes, aes(2, -4.6, group = Estimate), size = 2, shape = 19, color = "#FF00FF")
...
p_legend2 <- ggplot(Effect_sizes, aes(x = factor(Region, levels = c("Pre-critical", "Critical", "Spillover", "Post-spillover")), 
                               y = Z_value, color = Estimate, group = Estimate)) + geom_point(shape = 4, size = 3.5, stroke = 0.8) + ylim (-5, 5)
...
p_legend2 <- p_legend2 + labs(x="Region", y="Z-values", title = "Single Plausibility + GPT-2 Surprisal") 
p_legend2 <- p_legend2 + theme(legend.position="bottom", legend.text=element_text(size=10), legend.title=element_text(size=10), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), axis.text.x = element_text(size = 8)) + theme(plot.title = element_text(size=11))
...
combined_plot_with_legend <- grid.arrange(empty_row, combined_plot, legend, legend2, nrow = 4, heights = c(0.2,9,0.4,0.4))
combined_plot_with_legend <- grid.text(caption, x = 0.213, y = 0.98, just = "center", gp = gpar(fontsize = 15))
