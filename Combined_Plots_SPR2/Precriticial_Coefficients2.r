...
p1 <- p1 + labs(x="Region", y="SPR2 Coefficients", title = "Plausibility + GPT-2 Surprisal + Pre-critical RT") 
...
p2 <- p2 + labs(x="Region", y="SPR2 Coefficients", title = "Plausibility + LeoLM Surprisal + Pre-critical RT") 
p2 <- p2 + theme(legend.position="none", legend.text=element_text(size=7), legend.title=element_text(size=7), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), axis.text.x = element_text(size = 8)) + theme(plot.title = element_text(size=9)) + theme(plot.margin = margin(t = 0.5, r = 0.2, b = 0, l = 0.2, unit = "cm"))
p2 
...
combined_plot_with_legend <- grid.arrange(empty_row, combined_plot, legend, nrow = 3, heights = c(0.2,9,1))
combined_plot_with_legend <- grid.text(caption, x = 0.155, y = 0.97, just = "center", gp = gpar(fontsize = 15))
