...
#test <- GP6[Condition == "B" & (Plaus_target_avg >= 3 & Plaus_target_avg <= 5)]
test <- GP6[Condition == "C" & (Plaus_target_avg <= 2)]

#exclude for condition A values >5, for condition B <2 and >6 and for condition C >3
condition_A <- GP6$Condition == "A" & (GP6$Plaus_target_avg <= 5)
condition_B <- GP6$Condition == "B" & (GP6$Plaus_target_avg <= 3 | GP6$Plaus_target_avg >= 5)
condition_C <- GP6$Condition == "C" & (GP6$Plaus_target_avg >= 3)
...
GP6$Plaus_target_avg[(condition_A | condition_B | condition_C)] <- NA
...
setwd("~/Downloads/Master_Thesis/3_SPR_Study/Filtered_plausibility_ratings/SPR_Study/Results_Plaus_Plaus_avg/")
dt <- fread("GP6_filtered.csv") #plots plausratings after removing outliers


means <- aggregate(Plaus_target_avg ~ Condition, dt, FUN=mean)
means$Plaus_SE <- aggregate(Plaus_target_avg ~ Condition, dt, FUN=se)$Plaus_target_avg
dt_items_abc <- dt[, lapply(.SD, mean), by=list(Item, Condition), .SDcols=c("Plaus_target_avg")]
dt_items_abc <- dt_items_abc[!(Plaus_target_avg %in% c(NA)),] 

# density plot
p <- ggplot(dt_items_abc, aes(x=Plaus_target_avg, color=Condition, fill=Condition)) + geom_density(alpha=0.4) + theme_minimal() + ylim(0, 1.5)
p <- p + geom_vline(data=means, aes(xintercept=Plaus_target_avg, color=Condition), linetype="dashed") + scale_x_continuous(breaks=seq(1,7))
...
p <- p + labs(title = "Filtered Avg Plausibility", y="Density", x="Plausibility" )
ggsave("DensityPlot_Plausibility.pdf", p, device=cairo_pdf, width=4, height=4)
p

# barplot
q <- ggplot(means, aes(x=Condition, y=Plaus_target_avg)) + geom_bar(stat="identity") + labs(title = "Average Plausibility Ratings per Condition", y = "Plausibility",  x = "Condition") + geom_errorbar(aes(ymin=Plaus_target_avg-Plaus_SE, ymax=Plaus_target_avg+Plaus_SE), width=.4, position=position_dodge(.9)) + theme_minimal() + coord_cartesian(ylim = c(1, 7)) + scale_y_continuous(breaks = c(1:7))
ggsave("BarPlot_Plausibility.pdf", q, device=cairo_pdf, width=4, height=4)
q
