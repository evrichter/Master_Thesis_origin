...
for (region in regions) 
{
  ...
  for (condition in conditions)
  {
    ...
    # calculate standard error for RT estimated exponentiated
...
p <- ggplot(residuals, aes(x = factor(Region, levels = c("Pre-critical", "Critical", "Spillover", "Post-spillover")), 
                           y = Residual, color = Condition, group = Condition)) + geom_point(shape = 4, size = 3.5, stroke = 0.4) + geom_line(linewidth=0.5) + ylim (30, -20)
...
p <- p + labs(x="Region", y="RT", title = "Residuals raw-exp RTs (SPR)") 
...
ggsave("Residuals_SPR_single_raw_exp.pdf", p, width=4, height=4)
...
p <- p + scale_color_manual(name="Condition", labels=c("A: Plausible", "B: Medium Plausible", "C: Implausible"), values=c("#000000", "#FF0000", "#0000FF"))
p <- p + labs(x="Region", y="expRT", title = "Exponentiated Estimated RTs") 
p <- p + theme(legend.position="bottom", legend.text=element_text(size=7), legend.title=element_text(size=7), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14)) 
p 
ggsave("Estimated_exp_RTs_SPR_single.pdf", p, width=4, height=4)
