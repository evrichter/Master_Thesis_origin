...
p1 <- p1 + labs(x="Region", y="logRT", title = "A*") 
p1 <- p1 + theme(legend.position="none", legend.text=element_text(size=7), legend.title=element_text(size=7), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), axis.text.x = element_text(size = 8)) + theme(plot.title = element_text(size=11)) + theme(plot.margin = margin(t = 1, r = 0.2, b = 0, l = 0.2, unit = "cm"))
p1 
...
p2 <- p2 + labs(x="Region", y="logRT", title = "B*") 
...
combined_plot_with_legend <- grid.text(caption, x = 0.25, y = 0.97, just = "center", gp = gpar(fontsize = 15))
combined_plot_with_legend

setwd("~/Downloads/Master_Thesis/Plots_SPR2/")
# Save the combined plot
ggsave("Combined_Plot_Residuals.pdf", combined_plot_with_legend, device = "pdf")
